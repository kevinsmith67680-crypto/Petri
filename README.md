...
The `lobby` message carries `starts`, the seconds left on the count, because that message is what opens the card. Leaving the number to the next snapshot meant the card opened on whatever the *previous* count ended on — a "1" flashing where a "5" belongs — and showed a player joining mid-count nothing at all.
