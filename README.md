...
**Your own mass was the wrong colour.** Your cells are drawn in your own colour — the one picked in the lobby, or the default player colour — rather than straight from their palette slot, but ejected blobs used the slot — so your own mass came out looking like somebody else's. Blobs now carry an ownership flag and are drawn to match.
...
The `lobby` message carries `starts`, the seconds left on the count, because that message is what opens the card. Leaving the number to the next snapshot meant the card opened on whatever the *previous* count ended on — a "1" flashing where a "5" belongs — and showed a player joining mid-count nothing at all. During the count the Ready, Connected and Needed figures stay on the card — only the meter, which could only read full, gives way — and a line above the title names the room and its stake, taken from the `mode` the message already carries.

**The lobby is laid out like the pregame menu**: the shared header, two cards and the footer. *Last game* is on the left, where How it works sits on the menu, and *Next round* on the right, where Play sits. Next round runs lobby figures, then the ready button, then name and colour, so the one thing to do is high on the card: on a 1000×650 tablet it clears the fold even with the count showing. The cards are lighter than the menu's, so they stay side by side down to 760px rather than stacking at the menu's 1075px.

**Readiness survives from one round to the next, on both ends.** The server always kept it; the client reset its own flag at every round start, so the next lobby offered "I'm ready" to a player it was already counting in, and pressing it did nothing. The client now keeps it, takes a new connection's readiness from the welcome message, and drops it when the server sits a player out because it could not take the stake for another round.

**Return to menu** sits under the ready button and takes the player back to the pregame menu, with the practice arena behind it as on page load. It sends `leave` rather than just dropping the socket. A dropped socket lingers for six seconds before its stake is refunded, and nothing tells the player, so the menu they landed on showed the stake as still at risk. `leave` from a player with no body refunds the stake held for the round there and then, takes them out of the room and the world, pushes the account and closes normally; a live body gets no such exit and lingers like any other. Anything the old socket still delivers after the button is pressed is ignored except its account message, which is the one carrying the refund — a lobby broadcast already in flight would otherwise put the card back over the menu.

**Your cell is set up on the lobby's Next round card.** Name and colour sit below the ready button, beside a preview drawn the way the arena draws a cell. The name is the account's display name — the same one Rename changes in the menu — so it stays unique, validated and on the same 60-second cooldown, and a refusal is said on the card. Saving it keeps the connection: rename used to go through the sign-in path, which rebuilds the socket, and from the lobby that would have cost the player their place and their ready state. The colour is a palette slot (0–6), kept per device in `localStorage` under `engulfs.colour`, sent on join and changed with a `colour` message. The server takes a change only while the player has no body, which in practice means the lobby, so nobody can change their look mid-fight. Until a player picks, the server chooses and the welcome message says which, so the card shows the colour they actually have; practice keeps the default player colour.

**Colours travel with names.** Each name record in the snapshot carries the owner's palette slot (protocol 9). Before that the snapshot never said what colour anyone was: the client looked up `undefined` in the palette, the canvas silently ignored the fill, and every other player was painted in whatever had been filled last. A short browser run of the old build made over a thousand ignored fills; the same run now makes none. After a rename or a colour change the server forgets it told anyone, so the new record goes out on the next snapshot rather than at the next keyframe.

**`round_start` is not proof that you are alive.** The newest snapshot a client holds when the whistle blows was encoded during the count, when everyone is despawned — and from round two onwards the server is guaranteed to send one, because re-staking makes `startRound` await and a tick fits in the gap. A client that took `round_start` as "we are alive now" read that stale frame on its very next animation frame as having just been eaten, and threw up a death card carrying the *previous* round's orbs, peak and placing over a round that had only just opened. The client now arms its death check from a snapshot that actually shows it alive, never from an assumption; the same rule covers mid-round respawns, which are also a request rather than a fact.
